...
alertify.defaults.glossary.title = '';
		...
		$(document).on('click',".inCart", function(){
			...
			$bt = $(this);
			$prod = $bt.parent().parent();
			$('.producto',$prod).html();
			$('.producto',$prod).attr('producto-id');

			console.log($bt);
			console.log($prod);
			console.log($('.producto',$prod).html());
			console.log($('.producto',$prod).attr('producto-id'));
			console.log($('.producto',$prod).val());
			console.log($('.count',$prod).val());
			...
			$.ajax({
				...
				data:{ 
					...
					product_id: $('.producto',$prod).val(),
					quantity: $('.count',$prod).val()
					
				},
				success: function(data){
						alertify.confirm("Producto Agregado al Carrito",function (e) {
        if (e) {
            window.top.location.href= "/carts/ver";
        }
    }).set('labels', {ok:'Confirmar Compra', cancel:'Seguir Comprando'});
				},
